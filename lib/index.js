...
function Collectd(interval, host, port, hostname, securityLevel, username, password) {
    ...
    this.serverHosts = [];
    this.securityLevel = securityLevel || 1;   //TODO!!!!!!!!!!!!!!!! Throw exception if security level > 0 and username/password empty
    this.username = username || 'alice';
...
Collectd.prototype.send = function() {
	...
	var pkt = new Packet(this.write.bind(this), this.securityLevel, this.username, this.password);
...
function Packet(sendCb, securityLevel, username, password) {
	...
	if (securityLevel === 0) {
		...
		this.overhead = 36 + this.username.length;
	}
	else if (securityLevel === 2) {
		this.overhead = 42 + this.username.length;
	}
...
Packet.prototype = {
    send: function() {
	if (this.pos > 0) {
		this.buf = encapsulate(this.buf.slice(0, this.pos), this.securityLevel, this.username, this.password);
		this.buf = this.buf.slice(0, this.pos + this.overhead);
...
function encapsulate(pkt, securityLevel, username, password) {

	/* TODO:
	1. Merge decrypt into encapsulate PLUS insert trivial condition (securityLvl === 0 {return pkt})
	2. Corner Cases
	*/	
	var Buffer = require('buffer/').Buffer;
		...
		var buffer = new Buf(1024);

		//First two bytes 0x0200.
		buffer.writeUIntBE('0x0200',0,2);

		//length of username plus the rest overhead
		var STATIC_PACKAGE_OVERHEAD = 36; // 2 first bytes + length of overhead( = 2bytes) + length of mac (=32 bytes)
		var DYNAMIC_PACKAGE_OVERHEAD = username.length;
		var PACKAGE_OVERHEAD = STATIC_PACKAGE_OVERHEAD + DYNAMIC_PACKAGE_OVERHEAD;
		buffer.writeUIntBE(PACKAGE_OVERHEAD, 2, 2);
		...
		var macBuffer = new Buffer(username.length + pkt.length);
		macBuffer.write(username,0,username.length,'ascii');
		pkt.copy(macBuffer,username.length,0,pkt.length);

		//macBuffer to wordArray
		var macBufferWordArray = CryptoJS.enc.Hex.parse(macBuffer.toString('hex'));

		var mac = CryptoJS.HmacSHA256(macBufferWordArray, password);
		...
		//imput username in buffer
		buffer.write(username, STATIC_PACKAGE_OVERHEAD);

		//add pkt in the buffer
		pkt.copy(buffer,PACKAGE_OVERHEAD,0,pkt.length);

		//Return buffer converted from Buf class to Buffer class which is native in master.
		return bufToBuffer(buffer);
	}
		...
		//Input 0x0210 first 2 Bytes.
		buf.writeUIntBE('0x0210',0,2);

		//length of username plus the rest overhead
		var STATIC_PACKAGE_OVERHEAD = 36; // 2 first bytes + length of overhead( = 2bytes) + length of mac (=32 bytes)
		var DYNAMIC_PACKAGE_OVERHEAD = username.length + pkt.length;
		var PACKAGE_OVERHEAD = STATIC_PACKAGE_OVERHEAD + DYNAMIC_PACKAGE_OVERHEAD;
		buffer.writeUIntBE(PACKAGE_OVERHEAD, 2, 2);

		//length of name (int)
	    buffer.writeUIntBE(username.length,4,2);

	    //Input username
	    buffer.write(username,6);
	    buffer.write(CryptoJS.enc.Hex.stringify(iv), username.length + 6, 'hex');
        ...
        buffer.write(CryptoJS.enc.Hex.stringify(encryptedData.ciphertext), 22 + username.length, 'hex');
