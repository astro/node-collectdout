...
Collectd.prototype.send = function() {
	...
	var pkt = new Packet(this.write.bind(this), 1, 'alice', '12345678');
    ...
    console.log(pkt);
    pkt.buf = encapsulate(pkt.buf, this.securityLevel, this.username, this.password);
    console.log(pkt);
    pkt.send();
};
...
Packet.prototype = {
	...
	if (this.pos > 0) {
		...
		this.buf = this.buf.slice(0, this.pos + 41);
...
function encapsulate(pkt, securityLevel, uname, pass) {
	...
	*/	
	var Buffer = require('buffer/').Buffer;
		...
		//New buffer that holds information about uname+pkt. Then it creates a mac using this new buffer and the pass.
		var macBuffer = new Buffer(uname.length + pkt.length);
		macBuffer.write(uname,0,uname.length,'ascii');
		pkt.copy(macBuffer,uname.length,0,pkt.length);

		//macBuffer to wordArray
		var macBufferWordArray = CryptoJS.enc.Hex.parse(macBuffer.toString('hex'));

		var mac = CryptoJS.HmacSHA256(macBufferWordArray, pass);
		...
		//imput uname in buffer
		buffer.write(uname, STATIC_PACKAGE_OVERHEAD);
