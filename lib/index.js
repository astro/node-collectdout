...
Collectd.prototype.send = function() {
	...
	doubleHashForEach(p.gauges, function(type, typeInstance, values) {
	    pkt.catchOverflow(function() {
		addPrelude(type, typeInstance);
...
Packet.prototype = {
	...
	this.buf.writeUInt32BE(Math.abs(Math.floor(num / Math.pow(2, 32)) & 0xffffffff), this.pos);
	this.pos += 4;
	var sig = num < 0 ? 1 : 0;
	this.buf.writeUInt32BE((sig << 31) | (num & 0x7fffffff), this.pos);
	this.pos += 4;
    },
