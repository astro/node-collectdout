...
Collectd.prototype.send = function() {
    ...
    if (this.securityLevel === 0) {
    	...
    	var pkt = new Packet(this.write.bind(this), 36 + this.username.length);
    }

    else {
    	var pkt = new Packet(this.write.bind(this), 42 + this.username.length);
    }
...
function Packet(sendCb, lengthOverhead) {
    
    this.sendCb = sendCb;
    this.lengthOverhead = lengthOverhead || 0;
    this.buf = new Buffer(MAX_PACKET_SIZE - this.lengthOverhead);
    this.pos = 0;
}
